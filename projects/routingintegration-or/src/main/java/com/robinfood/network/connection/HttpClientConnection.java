package com.robinfood.network.connection;

import com.fasterxml.jackson.databind.JsonNode;
import com.robinfood.exceptions.ApiClientsException;
import com.robinfood.network.di.SimpleLoggingInterceptor;
...
import static com.robinfood.constants.Constants.JSON;
import static com.robinfood.enums.AppLogsTraceEnum.RESPONSE_DATA_ROUTING_INTEGRATION_OK;
...
public class HttpClientConnection {
    ...
    public String connectionProcess(
            String attribute, String token, String url
    ) {
     ...
     */
    public String connectionProcess(
            String attribute, Object requestBodyData, String token, String url
    ) {
        ...
        try (Response response = call.execute()) {
            ...
            if (!response.isSuccessful()) {
                ...
                throw new ApiClientsException(
                        ERROR_RESPONSE_HTTP.replaceComplement(
                                url + " Body : " + Objects.requireNonNull(response.body())
                                        .string())
                );
            }
            ...
            if (bodyToJson.isEmpty()) {
                LogsUtil.error(ERROR_RESPONSE_HTTP.getMessageWithCodeWithComplement(url));
                throw new ApiClientsException(ERROR_RESPONSE_HTTP_BODY_NULL.replaceComplement(url));
            }

            LogsUtil.info(RESPONSE_DATA_ROUTING_INTEGRATION_OK.getMessageWithCode(), bodyToJson);
    ...
    private Request buildHttp(Object dataBody, String token, String url) {
        ...
        Request.Builder requestBuilder = new Request.Builder().url(url).addHeader(AUTHORIZATION, token);

        if (Objects.isNull(dataBody)) {
            requestBuilder = requestBuilder.get();
        } else {
            requestBuilder = requestBuilder.post(RequestBody
                    .create(ObjectMapperSingleton.objectToJson(dataBody), JSON));
        }
