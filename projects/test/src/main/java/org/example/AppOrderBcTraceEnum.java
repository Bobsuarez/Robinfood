...
public enum AppOrderBcTraceEnum {
    ...
    SEARCH_FINAL_PRODUCTS("214", "Entered the search for final products"),

    NO_SEARCH_FOR_FINAL_PRODUCTS("215", "No search for final products"),
    ...
    public static String parseName(Enum info) {
//        System.out.println(info);
        return Arrays.stream(values()).filter(dat -> {
//            System.out.println("--- " +dat.name());
            return (dat.equals(info));
        }).findFirst().get().getMessageWithCode();
    }

    public static String parseCode(Enum info) {
//        System.out.println(info);
        return Arrays.stream(values()).filter(dat -> {
//            System.out.println("--- " +dat.name());
            return (dat.equals(info));
    ...
    public static String parseMessage(Enum info) {
//        System.out.println(info);
        return Arrays.stream(values()).filter(dat -> {
//            System.out.println("--- " +dat.name());
            return (dat.equals(info));
