...
BEGIN
    ...
    SET @message := CONCAT("[UPDATE] [orders] order_id -> ", NEW.id, " NEW.paid -> ", NEW.paid, " NEW.pos_id -> ", NEW.pos_id, " NEW.order_number", NEW.order_number, " NEW.order_invoice_number", NEW.order_invoice_number);
    ...
    IF(NEW.id != 0) THEN
        ...
        -- Get the consecutive numbers of the last order associated with the cash register, to reset the number of orders daily.
        SELECT
        ...
        -- If the invoice number is calculated by the backend (Legacy systems)
        IF(NEW.paid AND NEW.enabled_trigger = 0 AND _logOrderInvoiceNumber = 0) THEN
            ...
            SET @message = CONCAT(@message, ", The consecutive are incrementing from last order invoice number");
        END IF;
    END IF;
