...
import com.robinfood.repository.mocks.OrderStoreDTOMock;
...
public class OrdersStoreRepositoryTest {

    @Mock
    private OrderBcQueriesAPI orderBcQueriesAPI;

    @Mock
    private Call<APIResponseEntity<List<OrderStoreDTO>>> responseEntityCall;

    @InjectMocks
    private OrdersStoreRepository ordersStoreRepository;

    private final String token = "token";

    @Test
    void test_GetOrderStore_Should_OK_When_DataIsCorrect() throws Exception {

        when(orderBcQueriesAPI.getOrdersStore(
                anyLong(),
                any(LocalDate.class),
                any(LocalDate.class),
                anyString(),
        ...
        when(responseEntityCall.execute()).thenReturn(Response.success(
                new APIResponseEntity<>(
                        200,
                        OrderStoreDTOMock.getDataDefault(),
                        "CO",
        ...
        ordersStoreRepository.getDataOrderStore(
                DataOrderStoreRequestDTOMock.getDataDefault(),
                token
        );
    ...
    @Test
    void test_GetOrderStore_Should_InternalServerError_When_WrongAnswerOfOrderBC() throws Exception {
