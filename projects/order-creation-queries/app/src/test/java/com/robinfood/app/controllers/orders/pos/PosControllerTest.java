package com.robinfood.app.controllers.orders.pos;

import com.robinfood.app.mocks.PosResolutionDTOMock;
import com.robinfood.app.usecases.getposresolution.IGetPosResolutionUseCase;
import com.robinfood.app.usecases.getposresolutionbystore.IGetPosResolutionByStoreUseCase;
import com.robinfood.core.dtos.posresolution.DataPosResolutionRequestDTO;
import com.robinfood.core.enums.Result;
import org.apache.http.HttpHeaders;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.extension.ExtendWith;
import org.mockito.junit.jupiter.MockitoExtension;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.autoconfigure.web.servlet.AutoConfigureMockMvc;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.boot.test.mock.mockito.MockBean;
import org.springframework.http.HttpStatus;
import org.springframework.http.MediaType;
import org.springframework.test.context.TestPropertySource;
import org.springframework.test.web.servlet.MockMvc;
import org.springframework.test.web.servlet.request.MockMvcRequestBuilders;

import java.util.List;

import static com.robinfood.core.constants.APIConstants.ORDERS_V1;
import static org.mockito.ArgumentMatchers.any;
import static org.mockito.Mockito.when;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.status;

@AutoConfigureMockMvc
@SpringBootTest
@TestPropertySource(properties = {
        "jwt-token-prefix=Bearer ",
        "jwt-token-secret=gyXNfCp4Qx4Gn9OV7qK7uwRUX4f4bgrEEPvjdeLQFciHwtCKrc5rCm1kuNECIFP6",
        "jwt-token-aud=internal",
        "jwt-token-mod=order_creation_queries"
})
@ExtendWith(MockitoExtension.class)
class PosControllerTest {

    private static final String BEARER_AUTH = "Bearer ";
    private static final String
            TOKEN =
            "eyJhbGciOiJIUzUxMiIsInR5cCI6IkpXVCJ9" +
                    "..53cVBXL8PuMWcFLrCHJ6fyXCR64RlvxbSO39rxysXdVdFG-EA3hWxcGg2W897a0t6unFv8VjDmXAV0ExCwtH4A";
    @Autowired
    private MockMvc mockMvc;
    @MockBean
    private IGetPosResolutionUseCase getPosResolutionUseCase;
    @MockBean
    private IGetPosResolutionByStoreUseCase getPosResolutionByStoreUseCase;


    @Test
    void test_getPosResolution_Request_OK() throws Exception {

        when(getPosResolutionUseCase.invoke(any(DataPosResolutionRequestDTO.class)))
                .thenReturn(new Result.Success<>(PosResolutionDTOMock.getDataDefault()));

        mockMvc.perform(MockMvcRequestBuilders
                .get(ORDERS_V1 + "/pos-resolutions/1/sequence")
                .header(HttpHeaders.AUTHORIZATION, BEARER_AUTH + TOKEN)
                .queryParam("localDateEnd", "2023-01-17")
                .queryParam("localDateStart", "2023-01-16")
                .header("timeZone", "America/Bogota")
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
        ).andExpect(status().isOk());
    }

    @Test
    void test_getPosResolution_Should_BadRequest_When_EndPointGenerateError() throws Exception {
        when(getPosResolutionUseCase.invoke(any(DataPosResolutionRequestDTO.class)))
                .thenReturn(new Result.Error(new Exception("Some error"), HttpStatus.BAD_REQUEST));

        mockMvc.perform(MockMvcRequestBuilders
                .get(ORDERS_V1 + "/pos-resolutions/1/sequence")
                .header(HttpHeaders.AUTHORIZATION, BEARER_AUTH + TOKEN)
                .queryParam("localDateEnd", "2023-01-17")
                .queryParam("localDateStart", "2023-01-15")
                .header("timeZone", "America/Bogota")
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
        ).andExpect(status().isBadRequest());
    }

    @Test
    void test_getPosResolutionByStore_Request_OK() throws Exception {

        when(getPosResolutionByStoreUseCase.invoke(any(DataPosResolutionRequestDTO.class)))
                .thenReturn(new Result.Success<>(List.of(PosResolutionDTOMock.getDataDefault())));

        mockMvc.perform(MockMvcRequestBuilders
                .get(ORDERS_V1 + "/12/resolutions-sequence")
                .header(HttpHeaders.AUTHORIZATION, BEARER_AUTH + TOKEN)
                .queryParam("localDateEnd", "2023-01-17")
                .queryParam("localDateStart", "2023-01-17")
                .header("timeZone", "America/Bogota")
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
        ).andExpect(status().isOk());
    }

    @Test
    void test_getPosResolutionByStore_Should_BadRequest_When_EndPointGenerateError() throws Exception {
        when(getPosResolutionByStoreUseCase.invoke(any(DataPosResolutionRequestDTO.class)))
                .thenReturn(new Result.Error(new Exception("Some error"), HttpStatus.BAD_REQUEST));

        mockMvc.perform(MockMvcRequestBuilders
                .get(ORDERS_V1 + "/12/resolutions-sequence")
                .header(HttpHeaders.AUTHORIZATION, BEARER_AUTH + TOKEN)
                .queryParam("localDateEnd", "2023-01-17")
                .queryParam("localDateStart", "2023-01-17")
                .header("timeZone", "America/Bogota")
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
        ).andExpect(status().isBadRequest());
    }

}
