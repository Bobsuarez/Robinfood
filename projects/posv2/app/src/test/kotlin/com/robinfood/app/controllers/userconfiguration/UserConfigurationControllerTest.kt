...
class UserConfigurationControllerTest {
    ...
    @MockBean
    private lateinit var mockGetUserPosConfigurationUseCase: IGetUserPosConfigurationUseCase
    ...
    fun test_getUserConfiguration_Should_Return_Ok() {
        runBlocking {
            `when`(
                mockGetUserPosConfigurationUseCase.invoke(
                    BEARER_AUTH + TOKEN,
                    userId,
                )
            ...
            val result = mockMvc.perform(
                MockMvcRequestBuilders
                    .get("$API_V1$USER/1/pos-configuration")
                    .header(HttpHeaders.AUTHORIZATION, BEARER_AUTH + TOKEN)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(userConfigurationResponseDTO.toJson())
    ...
    @Test
    @Throws(Exception::class)
    fun test_getPosStore_Should_Return_Failure() {
        runBlocking {
            `when`(
                mockGetUserPosConfigurationUseCase.invoke(
                    BEARER_AUTH + TOKEN,
