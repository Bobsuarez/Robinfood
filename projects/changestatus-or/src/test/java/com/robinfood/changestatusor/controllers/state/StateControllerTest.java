...
public class StateControllerTest {
    ...
    void test_changeStatusOrder_When_Change_Order_State_Then_Return_Status_Change_Successful() throws Exception {
        ...
        when(changeOrderStatusUseCase.invoke(any(ChangeOrderStatusDTO.class)))
                .thenReturn(ChangeOrderStatusDTOMock.getDefault());

        mockMvc.perform(MockMvcRequestBuilders
                        .post(URL + STATE_V1)
                        .header(HttpHeaders.AUTHORIZATION, BEARER_AUTH + TOKEN)
                        .content(new Gson().toJson(changeOrderStatusDTO))
