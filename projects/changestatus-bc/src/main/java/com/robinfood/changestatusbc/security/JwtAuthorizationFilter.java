package com.robinfood.changestatusbc.security;

import com.robinfood.changestatusbc.dtos.ApiResponseDTO;
import com.robinfood.changestatusbc.dtos.UserDTO;
import com.robinfood.changestatusbc.exceptions.AuthException;
...
import static com.robinfood.changestatusbc.configs.constants.GlobalConstants.UNAUTHORIZED_REQUEST;
...
public class JwtAuthorizationFilter extends OncePerRequestFilter {
    ...
    private void denyRequest(@NotNull final HttpServletResponse response) {
        try (PrintWriter writer = response.getWriter()) {
            ApiResponseDTO<Object> apiResponse = ApiResponseDTO.builder()
                    ...
                    .error(Boolean.FALSE)
                    .build();
            final String backOfficeResponse = objectToJson(apiResponse);
    ...
    private void sendErrorResponse(@NotNull final HttpServletResponse response, String msg, String stackTrace) {
        try (PrintWriter writer = response.getWriter()) {
            ApiResponseDTO<Object> apiResponse = ApiResponseDTO.builder()
                    ...
                    .message(String.format("%s: %s", msg, stackTrace))
                    .error(Boolean.FALSE)
                    .build();
    ...
    private Set<SimpleGrantedAuthority> getGrantedValues(Claims claims) {
        ...
        try {
            List<String> scopes =  getPermissions(claims);
            if (scopes != null) {
    ...
    private static List<String> getPermissions(Claims claims) {
        return (List<String>) claims.get("per");
    }
}
