...
public interface IOrderHistoryController {
     ...
     *
     * @param uuid uuid of a order
     * @return Order status history
     */
    @Operation(summary = "Get order status history")
    @ApiResponse(
            responseCode = "200", description = "Order status history information",
            content = {
                    @Content(
                            mediaType = MediaType.APPLICATION_JSON_VALUE,
                            schema = @Schema(implementation = ApiResponseDTO.class)
    ...
    ResponseEntity<ApiResponseDTO<List<OrderStatusHistoryDTO>>> getOrderStatusHistory(
