...
public class AndPredicateStrategy<T> implements IQueryStatementStrategy<T> {

    @Override
    public Predicate buildPredicate(CriteriaBuilder criteriaBuilder, Root<T> root, SearchToCriteriaDTO searchToCriteriaDTO) {
        return criteriaBuilder.and(criteriaBuilder.equal(root.get(searchToCriteriaDTO.getKey()), searchToCriteriaDTO.getValue()));
    }
}
