...
class GetOrderDetailUseCaseTest {
    ...
    @Mock
    private IGetOrderServicesUseCase getOrderServicesUseCase;
    ...
    private final OrderThirdPartyEntity orderThirdPartyEntity = new OrderThirdPartyEntity(
            null,
            null,
            "22675323",
            1L,
    ...
    void test_GetOrderDetailOrderUseCase_Returns_Correctly() {
        ...
        when(orderPosRepository.findByPosIdAndCurrent(anyLong(), anyInt())).thenReturn(
                Optional.ofNullable(PosResolutionEntityMock.getDataDefault())
        );

        when(getOrderServicesUseCase.invoke(anyLong())).thenReturn(
                GetOrderServicesDetailsDTOMock.getDefaultList()
        );
        ...
        final List<GetOrderDetailDTO> result = getOrderDetailUseCase.invoke(
                orderIds
        );
