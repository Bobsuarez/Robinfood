...
public class GetOrderDiscountByOrderIdsUseCaseTest {

    @Mock
    private IOrderDiscountCRUDRepository orderDiscountCRUDRepository;

    @InjectMocks
    private GetOrderDiscountByOrderIdsUseCase getOrderDiscountByOrderIdsUseCase;

    private final List<Long> orderIds = new ArrayList<>(Arrays.asList(1L, 2L, 3L));
    ...
    @Test
    void test_GetOrderDiscountsByOrderIds_Returns_Correctly() {

        when(orderDiscountCRUDRepository.findOrderDiscountEntitiesByOrderIdInAndOrderFinalProductIdIsNull(orderIds))
                .thenReturn(orderDiscountEntities);

        final List<OrderDiscountDTO> result = getOrderDiscountByOrderIdsUseCase.invoke(orderIds);
